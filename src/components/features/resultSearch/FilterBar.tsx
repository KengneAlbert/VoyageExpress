...
const FilterBar: React.FC<FilterBarProps> = ({ onFilterChange, onSort }) => {
  ...
  const [selectedFilters, setSelectedFilters] = useState<
    Record<"price" | "time" | "agency", string>
  >({
    price: "",
    time: "",
    agency: "",
    ...
    <>
      <motion.div
        ...
        className="relative bg-gradient-to-r from-gray-900/90 via-gray-800/90 to-gray-900/90 
                   backdrop-blur-xl rounded-lg sm:rounded-xl p-3 sm:p-4 shadow-xl border border-gray-800/50
                   w-full overflow-hidden"
      >
        <div className="flex items-center justify-between gap-2 sm:gap-4">
          <div
            className="flex-1 overflow-x-auto scrollbar-hide snap-x snap-mandatory 
                        flex items-center gap-2 sm:gap-4 pr-2 sm:pr-4 relative"
          >
            <div className="flex-none flex items-center gap-1.5 sm:gap-2 text-orange-400 px-2 sm:px-3 snap-start">
                ...
                onClick={() =>
                  setActiveFilter(
                    activeFilter === filter.id
                      ? ""
                      : (filter.id as "price" | "time" | "agency")
                  )
                }
                className={`flex-none snap-start flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-4 py-1.5 sm:py-2 
                          rounded-md sm:rounded-lg transition-all text-xs sm:text-sm
                  ${
                    activeFilter === filter.id
          ...
          {hasActiveFilters && (
            <motion.button
              ...
              className="flex-none ml-2 sm:ml-4 px-2 sm:px-3 py-1.5 sm:py-2 flex items-center gap-1 sm:gap-2 
                       text-xs sm:text-sm text-gray-400 hover:text-white rounded-md sm:rounded-lg 
                       bg-gray-800/50 hover:bg-gray-700/50 border border-gray-700 transition-all"
            >
          ...
          <>
            ...
            <motion.div
              ...
              className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 
                        w-[calc(100vw-2rem)] sm:w-full max-w-md z-50 mx-4"
            >
              <div
                className="bg-gradient-to-b from-gray-900 to-gray-800 rounded-xl 
                  ...
                  {filters
                    ...
                    ?.options.map((option) => (
                      <motion.button
                        ...
                        onClick={() =>
                          handleOptionSelect(
                            activeFilter as "price" | "time" | "agency",
                            option
                          )
                        }
                      ...
                      >
                        ...
                        {selectedFilters[
                          activeFilter as keyof typeof selectedFilters
                        ] === option && <Check className="h-4 w-4" />}
                      </motion.button>
                    ))}
                </div>
              </div>
