...
import { MapPin, Calendar, Clock, User, Download, Printer, Bus, CheckCircle, Shield, Info, Share2, Home } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] 
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <motion.div className="text-center mb-4 sm:mb-8 relative">
          ...
          <h1 className="text-2xl sm:text-4xl font-bold text-white mb-2 sm:mb-3">Paiement réussi !</h1>
        ...
        <div ref={ticketRef} className="relative">
          ...
          <div className="absolute top-4 sm:top-6 right-12 sm:right-20 w-12 h-12 sm:w-20 sm:h-20">
            <div className="relative w-full h-full">
              <div className="absolute inset-0 rounded-full bg-gradient-to-r from-orange-500/20 to-orange-600/20
              ...
              <div className="absolute inset-2 rounded-full bg-gray-900/90 flex items-center justify-center">
                <span className="text-xs font-bold text-orange-500 tracking-wider">SECURE</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-gray-800/80 to-gray-900/80 
            ...
            <div className="absolute top-1/2 -left-4 w-8 h-8 bg-gray-900 rounded-full transform -translate-y-1/2
                         before:absolute before:inset-1 before:bg-gray-800 before:rounded-full" />
            <div className="absolute top-1/2 -right-4 w-8 h-8 bg-gray-900 rounded-full transform -translate-y-1/2
                         before:absolute before:inset-1 before:bg-gray-800 before:rounded-full" />
            ...
            <div className="p-3 sm:p-6 border-b border-white/10">
              <div className="flex items-center justify-between gap-2 sm:gap-4">
                <div className="flex items-center gap-2 sm:gap-4">
                  <img src={bookingData.trip.logo} alt={bookingData.trip.agency}
                      className="w-10 h-10 sm:w-16 sm:h-16 rounded-lg object-cover" />
                  <div>
                    <h2 className="text-base sm:text-xl font-bold text-white">{bookingData.trip.agency}</h2>
                    <div className="flex items-center gap-1 sm:gap-2 text-orange-400">
                      <Bus className="w-3 h-3 sm:w-4 sm:h-4" />
                      <span className="text-xs sm:text-sm">N° {Math.random().toString(36).substr(2, 9).toUpperCase()}</span>
                    </div>
                  </div>
                </div>
            ...
            <div className="p-3 sm:p-6 space-y-4 sm:space-y-6">
              ...
              <div className="space-y-3 sm:space-y-4">
                <div className="flex items-center gap-2 text-gray-400">
                  ...
                  <span className="text-xs sm:text-sm font-medium">Passagers et Places</span>
                </div>
                <div className="grid gap-3 sm:gap-4">
                  {bookingData.passengers.map((passenger: Passenger, index: number) => (
                    <div key={index} 
                      ...
                      <span className="px-3 py-1.5 sm:px-4 sm:py-2 bg-orange-500/10 
              ...
              <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="font-medium text-orange-400">Informations importantes</h4>
                </div>
            ...
            <div className="px-3 py-2 sm:px-6 sm:py-4 bg-gray-900/50 border-t border-gray-700/50">
              <div className="flex flex-col sm:flex-row items-center justify-between 
                ...
                <span className="font-mono">{`${new Date().getFullYear()}${Math.random().toString(36).substr(2, 9).toUpperCase()}`}</span>
              </div>
            </div>
          </div>
        </div>
